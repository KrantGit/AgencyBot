...
Пользователь привязывает Telegram командой `/start <логин> <пароль>`. После привязки доступны `/orders`, `/neworder` (администратор) и `/help`. Создание заказа — пошаговый сценарий; черновик хранится в Redis 30 минут, а `/cancel` отменяет его. Не отправляйте пароль в общие чаты. Notification Service читает события пользователей и заказов из Kafka и отправляет привязанным активным исполнителям сообщения о новых изменениях.
